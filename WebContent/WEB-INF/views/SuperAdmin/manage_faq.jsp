...
<div class="col-md-9 col-sm-9 col-xs-12 pad0">
	...
	<a href="admin/addFaq/" class="btn btn-primary btn-submit" style="float:right; margin:12px ">Add New</a>
	<% } %>
	...
	<table id="example" class="mdl-data-table" cellspacing="0" width="100%">
    	...
    	<c:set var="h_id" value="${faq.faq_id}"/>
<%
String str=pageContext.getAttribute("h_id").toString();
byte[] encodedBytes = Base64.encodeBase64(str.getBytes());
%>
    	<div class="btn-group">
          <a title="Edit" class="btn btn-danger" href="admin/editFaq?q=<%=new String(encodedBytes)%>"><i class="fa fa-pencil-square-o edit"></i></a>
          <a title="Delete" class="btn btn-danger" onClick="confirmBox(this.id)" id="${faq.faq_id}" href="javascript:void(0);"><i class="fa fa-trash-o delete"></i></a>
        </div>
    </td>
    	<% } %>
...
function confirmBox(id)
{
	swal({
		...
		if (isConfirm)
			window.location="admin/deleteFaq?faq_id="+id;
	});
}
