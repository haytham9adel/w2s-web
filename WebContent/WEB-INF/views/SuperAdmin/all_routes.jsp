...
<div class="col-md-9 col-sm-9 col-xs-12 pad0">
	...
	<a href="admin/addRoute" class="btn btn-primary btn-submit" style="float:right; margin:12px ">Add New</a>
	<% } %>
	...
	<table id="example" class="mdl-data-table sep_row"  cellspacing="0" width="100%">
    	...
    	<td style="min-width:93px !important;">
    	 <c:set var="id" value="${staff.route_id}"/>
    	
    	<%
	String str=pageContext.getAttribute("id").toString();
	...
	%>	
			...
			<div class="btn-group">
	          <a title="View" class="btn btn-danger"  href="admin/viewRoute?route_id=<%=new String(encodedBytes)%>"><i class="fa fa-eye edit"></i>	</a>
		    	 <% if((Integer)session.getAttribute("permission")==1) {%>
		    	<a title="Edit" class="btn btn-danger"  href="admin/editRoute?id=<%=new String(encodedBytes)%>"><i class="fa fa-pencil-square-o edit"></i></a>
...
function confirmBox(id)
{
	swal({
		...
		if (isConfirm)
			window.location="admin/deleteRoute?route_id="+id;
	});
}
