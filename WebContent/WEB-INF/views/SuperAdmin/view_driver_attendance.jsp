...
<script>
    ...
    $(document).ready(function() {
    	...
    	$(".k-btn-previous-year").click(function(){
  		  checkValue();
  	   });
	  	$(".k-btn-previous-month").click(function(){
	  		checkValue();
	  	});
	  	$(".k-btn-next-year").click(function(){
	  		checkValue();
	  	});
    });
...
<div class="col-md-9 col-sm-6 col-xs-12 m-side">

				<h2><%-- ${student.s_fname} ${student.s_lname}  --%></h2>
				<div class="report_box col-sm-12 pad0" style="margin-bottom:15px;">
				&nbsp;
             ...
             <div class="col-sm-12 col-xs-12 new-student-form view_student_detail pad0"> 
		...
		<div class="col-md-12 pad0">
			<div class="tabbable" id="tabs-637212">
				...
				<div class="tab-content">
					...
					<script>
					$(document).ready(function(){
					var start = new Date("${all_present[0].login_date}"),
				    end = new Date(),
					...
					for(var j=dates.length-2;j>-1;j--)
						...
						if(convert1(dates[j])=="${present.login_date}")
						{
							status="Present";
							aphStatus="success";
							aphStatus1="#D0E9C6";
						...
						if(convert1(dates[j])==convert1(new Date()))
							...
							htmlData1 +='<td style="background:'+aphStatus1+';">'+status+'</td>';
							htmlData1 +='</tr>'; */
						htmlData +='<tr class="'+aphStatus+'" style="background:'+aphStatus+';">';
						htmlData +='<td>'+parseInt(j+1)+'</td>';
						htmlData +='<td>'+convert(dates[j])+'</td>';
			...
			<table class="table table-hover" style="display:none;" id="example1">
				<thead>
					<tr>
						...
						<th colspan="2" text-align="center">Morning</th>
						<th colspan="2" text-align="center">Evening</th>
...
function checkValue()
{
	...
	<c:forEach items="${all_present}" var="present">
   $('.k-in-month[data-date="${present.login_date}"]').addClass('present');
   </c:forEach>
   ...
   $('.k-out-of-month[data-date="${holiday.holiday_date}"]').addClass('holi');
	...
	<c:forEach items="${all_present}" var="present">
  $('.k-out-of-month[data-date="${present.login_date}"]').addClass('present');
  </c:forEach>
...
var dateArr=[<c:forEach items="${all_present}" var="present">"${present.login_date}",</c:forEach>];
...
$(document).ready(function(){
	$(".pmu-instance:nth-child(3)").click(function(){
		...
		$.ajax({
		    url : 'getTotalDriverPresentAbsent.html',
		    type:'post',
		    contentType: "application/json; charset=utf-8",
		    dataType: "json",
		    data: JSON.stringify({"current_month":current_month,"student_id":"${student.driver_id}",school_id:"<%=(Integer)session.getAttribute("new_school_id")%>"}),
	  ...
	  $( ".datepicker1" ).datepicker({
	        ...
	        onSelect : function(selected_date){
	          ...
	          var endDate = new Date(selectedDate.getTime() + msecsInADay);
	           ...
	           $.ajax({
		   		    url : 'getTotalDriverPresentAbsentByDateRange.html',
		   		    type:'post',
		   		    contentType: "application/json; charset=utf-8",
		   		    dataType: "json",
		   		    data: JSON.stringify({"start_date":$("#start_date").val(),"end_date":selected_date,"student_id":"${student.driver_id}",school_id:"<%=(Integer)session.getAttribute("new_school_id")%>"}),
		   		    success : function(response) {
		   		    	$("#hd_1").html(response['holiday_days']);
		   		    	$("#hd_2").html(response['present_day']);
		   		    	$("#hd_3").html(response['absent_day']);
