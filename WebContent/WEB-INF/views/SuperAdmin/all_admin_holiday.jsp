...
<div class="col-md-9 col-sm-9 col-xs-12 pad0">
	...
	<% if((Integer)session.getAttribute("permission")==1) {%>
		<a href="admin/addAdminHoliday/" class="btn btn-primary btn-submit"
			style="float: right; margin: 12px">Add New</a> 
			<% } %>
		...
		<table id="example" class="mdl-data-table" cellspacing="0" width="100%"
			...
			<thead>
				<tr>
					...
					<th><input type="checkbox" class="checkbox" id="select_all" style="margin-left: 8px;">
						<div class="btn-group">
							<a href="javascript:void(0);" type="button" id="delete-all-btn" class="btn btn-danger">
			...
			<tbody>
				...
				<c:forEach items="${all_holiday}" var="staff">
					<tr>
						...
						<td><c:out value="${staff.holiday_name}" /></td>
						<td><c:out value="${staff.holiday_date}" /></td>
						<td><c:out value="${staff.holiday_enddate}" /></td>
						<% if((Integer)session.getAttribute("permission")==1) {%>
						<td><c:set var="h_id" value="${staff.h_id}" /> <%
String str=pageContext.getAttribute("h_id").toString();
byte[] encodedBytes = Base64.encodeBase64(str.getBytes());
%> <a title="Edit" href="admin/editAdminHoliday?q=<%=new String(encodedBytes)%>"><i
								class="fa fa-pencil-square-o fa-2x edit"></i></a> <%-- <a 
onClick="return confirmBox(this.id);" id="${staff.h_id}" href="javascript:void(0);"><i class="fa fa-times fa-2x delete"></i></a>
		 --%></td>
						
						<td><input type="checkbox" class="checkbox" name="delete"
							id="${staff.school_id}" value="${staff.h_id}"></td>
							<% } %>
					</tr>
...
<form action="admin/deleteMultipleAdminHoliday" method="post"
	id="send-form">
