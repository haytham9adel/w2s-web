...
<div class="col-md-9 col-sm-6 col-xs-12 m-side">
				...
				<div class="report_box col-sm-12" style="margin-bottom:15px;">
				&nbsp;
...
$(document).ready(function(){
	$(".pmu-instance:nth-child(3)").click(function(){
		...
		$.ajax({
		    ...
		    dataType: "json",
		    data: JSON.stringify({"current_month":current_month,"student_id":"${student.driver_id}",school_id:"<%=(Integer)session.getAttribute("schoolId")%>"}),
	        ...
	        onSelect : function(selected_date){
	          ...
	          var endDate = new Date(selectedDate.getTime() + msecsInADay);
	           ...
	           $.ajax({
		   		    ...
		   		    contentType: "application/json; charset=utf-8",
		   		    dataType: "json",
		   		    data: JSON.stringify({"start_date":$("#start_date").val(),"end_date":selected_date,"student_id":"${student.driver_id}",school_id:"<%=(Integer)session.getAttribute("schoolId")%>"}),
		   		    success : function(response) {
		   		    	$("#hd_1").html(response['holiday_days']);
		   		    	$("#hd_2").html(response['present_day']);
		   		    	$("#hd_3").html(response['absent_day']);
