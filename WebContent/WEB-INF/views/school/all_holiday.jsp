...
<div class="col-md-9 pad0">
	...
	<a href="school/addHoliday/" class="btn btn-primary btn-submit" style="float:right; margin:12px ">Add New</a>
	<% } %>
	...
	<table id="example" class="mdl-data-table" cellspacing="0" width="100%">
        <thead>
         ...
         <c:set var="count1" value="0" scope="page" />
			  ...
			  <c:forEach items="${all_holiday}" var="staff">
				  ...
				  <c:forEach items="${deleted_holiday}" var="staff1">
						
						<c:choose>
							<c:when test="${staff1.holiday_id==staff.h_id}">
		...
		%>
    	<a title="Edit" href="school/editHoliday?q=<%=new String(encodedBytes)%>"><i class="fa fa-pencil-square-o fa-2x edit"></i></a>
...
<form action="school/deleteMultipleHoliday" method="post" id="send-form">
...
function confirmBox(id)
{
	swal({
		...
		if (isConfirm)
			window.location="school/deleteHoliday?h_id="+id;
	});
}
...
function confirmBox1(id)
{
	swal({
		...
		if (isConfirm)
			window.location="school/deleteAdminHoliday?h_id="+id;
	});
}
