		...
		<form class="form-horizontal" id="student" method="post">
			<%--   <div class="form-group">
                     <label class="col-sm-3 control-label">School:</label>
                       <div class="col-sm-9">
                       <select class="form-control" id="s_school_id" name="s_school_id" onchange="return getParent(this.value);">
                         <option value="">Select School</option>
                         <c:if test="${!empty schools}">
                         	<c:forEach items="${schools}" var="school">
                         	<option  	<c:if test="${school.s_id==studentBean.s_school_id}">
                         	<c:out value="selected"/>
                         	 </c:if>   value=<c:out value="${school.s_id}"/>><c:out value="${school.school_name}"/></option>
                         	</c:forEach>
                         </c:if>
                         </select>
                      </div>
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<input type="text" name="s_fname" value="${studentBean.s_fname}"
						...
						value="<%=session.getAttribute("schoolId")%>"
						class="form-control">
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<select class="form-control" name="blood_type" id="blood_type">
						...
						<option <c:if test="${studentBean.blood_type=='A+'}">
    selected="selected"
</c:if>  value="A+">A+</option>
						<option <c:if test="${studentBean.blood_type=='A-'}">
    selected="selected"
</c:if> value="A-">A-</option>
						<option <c:if test="${studentBean.blood_type=='B+'}">
    selected="selected"
</c:if> value="B+">B+</option>
						<option <c:if test="${studentBean.blood_type=='B-'}">
    selected="selected"
</c:if>  value="B-">B-</option>
						<option <c:if test="${studentBean.blood_type=='AB+'}">
    selected="selected"
</c:if>   value="AB+">AB+</option>
						<option <c:if test="${studentBean.blood_type=='AB-'}">
    selected="selected"
</c:if>    value="AB-">AB-</option>
						<option <c:if test="${studentBean.blood_type=='O+'}">
    selected="selected"
</c:if>    value="O+">O+</option>
						<option <c:if test="${studentBean.blood_type=='O-'}">
    selected="selected"
</c:if>  value="O-">O-</option>
						...
						<div class="profile-thumb">
							...
							<c:choose>
								<c:when test="${studentBean.s_image_path!=''}">
									<td><img id="blah" alt="s_image_path"
										class="manage-icon-big"
										src="<%=Assets.STUDENT_UPLOAD_PATH_DIS %>/${studentBean.s_image_path}" />
								</c:when>
								<c:otherwise>
			...
			<div class="form-group">
				<div class="col-sm-9">
					...
					<input class="btn btn-primary btn-submit" value="Save" id="save"
						...
						href="school/manageStudents"> <input
...
$('#cancel').on('click', function(e) {
	...
	swal({
		...
		if (isConfirm)
			window.location = "school/manageStudents";
	});
})	 
});
