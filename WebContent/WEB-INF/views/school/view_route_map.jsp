...
<script
	src="https://maps.googleapis.com/maps/api/js?sensor=false&libraries=drawing&key=AIzaSyC7CzVRaX4toLFXcXnNV_8TUtgRy2Y0efE"></script>
...
['${school_details.school_name}','${route.source_lat}','${route.source_lng}','resources/dashboard/Images/source_icon.png'],		                  
...
[<c:choose><c:when test="${latlng.s_image!=''}">'<div class="map_info-win"><img  src="<%=Assets.STUDENT_UPLOAD_PATH_DIS %>/${latlng.s_image}" /></c:when><c:otherwise>'<div class="map_info-win"><img src="resources/dashboard/Images/user_icon.png" /></c:otherwise></c:choose><div><p>Name : <a class="map-link-info" href="school/viewStudent?q=<%=new String(encodedBytes)%>">${latlng.student_name}</a></p><p>Class : ${latlng.s_class}</p></div></div>',"${latlng.lat}","${latlng.lng}",'<c:choose><c:when test="${latlng.s_image!=''}"><%=Assets.STUDENT_UPLOAD_PATH_DIS %>/${latlng.s_image}	</c:when><c:otherwise>resources/dashboard/Images/user_icon.png</c:otherwise></c:choose>'],
</c:forEach>        
['Destination','${route.destination_lat}','${route.destination_lng}','resources/dashboard/Images/desti.png'],		                   
 ];
